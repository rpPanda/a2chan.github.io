...
const observer = new IntersectionObserver(function (entries, observer) {
	entries.forEach(entry => {
		...
		if (entry.target.className == "writer animated") {
			...
			writer_title.style.animation = "padding_10px_yellow 1s ease-in-out 1.5s forwards";
			writer_text.style.animation = "bg_lyellow 1s ease-in-out 1.5s forwards";
		}
		else {
			entry.target.classList.toggle("fadeInRight");
			client_title.style.animation = "padding_10px_blue 1s ease 1.5s forwards";
			client_text.style.animation = "bg_lblue 1s ease-in-out 1.5s forwards";
		}
